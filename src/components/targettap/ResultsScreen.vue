<template>
  <div class="results">
    <h1>Time!</h1>
    <p class="difficulty-name">{{ difficultyLabel }} · Target {{ results.targetLetter }}</p>
    ...
    <div class="stats-grid">
      <div class="stat">
        <span class="label">Median Hit RT</span>
        <span class="value">{{ Math.round(results.medianHitRT) }}ms</span>
      </div>
      <div class="stat">
        <span class="label">Average Hit RT</span>
        <span class="value">{{ Math.round(results.avgHitRT) }}ms</span>
      </div>
    </div>
...
const { recordCompletion, getStats } = useTargetTapStats()
const { isNewBestScore, isNewBestHitRate, isNewBestFalseAlarmRate, isNewBestMedianRT } =
...
const difficultyLabel = TARGETTAP_DIFFICULTIES[props.difficultyKey]?.label

const newBestParts = [
  isNewBestScore && 'Score',
  isNewBestHitRate && 'Hit Rate',
  isNewBestFalseAlarmRate && 'False Alarm Rate',
