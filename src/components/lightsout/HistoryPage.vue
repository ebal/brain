<template>
  <div class="history">
    ...
    <!-- 50 levels don't fit a button row the way Hanoi's 6 do — a plain
         select is the simplest usable picker at this count. -->
    <select class="level-select" v-model.number="activeLevel">
      <option v-for="lvl in levels" :key="lvl.level" :value="lvl.level">
        Level {{ lvl.level }} ({{ lvl.size }}×{{ lvl.size }})
      </option>
    </select>
...
const { getHistory, getStats } = useLightsOutStats()
